...
static esp_lcd_touch_handle_t tp = NULL;              /*Type of LCD touch panel handle*/
static esp_lcd_panel_io_handle_t tp_io_handle = NULL; /*Type of LCD panel io handle*/
...
esp_err_t touch_init(void)
{
    ...
    esp_lcd_panel_io_i2c_config_t io_config = {
        .dev_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP, /*Primary I2C device address attempt*/
        .control_phase_bytes = 1,                              /*1 byte for control phase*/
        .dc_bit_offset = 0,                                    /*No DC bit offset (not used for touch)*/
        .lcd_cmd_bits = 16,                                    /*16-bit command width*/
        .flags = {
            .disable_control_phase = 1, /*Disable control phase for touch controllers*/
        },
        .scl_speed_hz = 400000, /*I2C clock speed: 400kHz (Fast Mode)*/
    }; /*I2C configuration for touch panel controller (GT911)*/
    esp_lcd_touch_config_t tp_cfg = {
        .x_max = H_size,                /*Maximum X coordinate (horizontal resolution)*/
        .y_max = V_size,                /*Maximum Y coordinate (vertical resolution)*/
        .rst_gpio_num = Touch_GPIO_RST, /*Reset GPIO pin number*/
        .int_gpio_num = Touch_GPIO_INT, /*Interrupt GPIO pin number*/
        .levels = {
            .reset = 0,     /*Active low reset level*/
            .interrupt = 0, /*Active low interrupt level*/
        },
        .flags = {
            .swap_xy = false,  /*Don't swap X and Y coordinates*/
            .mirror_x = false, /*Don't mirror X axis*/
            .mirror_y = false, /*Don't mirror Y axis*/
        },
    }; /*Touch panel configuration parameters*/
    /*Initialize I2C communication with touch controller*/
    err = esp_lcd_new_panel_io_i2c((i2c_master_bus_handle_t)i2c_bus_handle, &io_config, &tp_io_handle);
    if (err != ESP_OK)
        return err;
    err = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, &tp); /*Initialize GT911 touch controller with primary address*/
    if (err != ESP_OK)  /*If primary address fails, try backup I2C address*/
    {
        io_config.dev_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS; /*Backup I2C device address*/
        err = esp_lcd_new_panel_io_i2c((i2c_master_bus_handle_t)i2c_bus_handle, &io_config, &tp_io_handle);
        if (err != ESP_OK)
            return err;

        err = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, &tp); /*Initialize GT911 touch controller with backup address*/
        if (err != ESP_OK)
            return err;
    }
    return err;
...
static esp_err_t lvgl_init()
{
    ...
    const lvgl_port_touch_cfg_t touch_cfg = {
        .disp = my_lvgl_disp,
        .handle = tp, /*LCD touch IO handle*/
    };
    my_touch_indev = lvgl_port_add_touch(&touch_cfg); /*Add LCD touch as an input device*/
